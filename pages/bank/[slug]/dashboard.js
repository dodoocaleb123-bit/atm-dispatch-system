...
export default function DynamicBankDashboard() {
  ...
  useEffect(() => {
    ...
    async function loadBankData() {
      ...
      // 1. Fetch current bank details by slug
      ...
      if (bankError || !bankData) {
        console.error('Error loading bank:', bankError);
        setLoading(false);
        return;
      }
      ...
      // 2. Fetch ONLY this bank's tickets
  ...
  if (!bank) return <div className="p-8 text-red-500">Bank not found or unauthorized.</div>;
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 p-8">
      ...
      <section>
        <h2 className="text-xl font-semibold mb-4">Your Active Service Tickets ({tickets.length})</h2>
        {tickets.length === 0 ? (
          ...
          <div className="grid gap-4">
            {tickets.map((ticket) => (
              <div key={ticket.id} className="p-4 bg-slate-900 border border-slate-800 rounded-lg">
                ...
                <span className="inline-block mt-2 px-2 py-1 text-xs bg-purple-950 text-purple-300 border border-purple-800 rounded">
